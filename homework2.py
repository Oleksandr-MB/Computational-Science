...
class Homework2:
    ...
    def test(self):
        ...
        print(f"Close to expected?\n"
                "-- Rosenbrock Function: --\n"
                f"Objective function (no optimization): {np.allclose(m1_vanilla, m1_expected, rtol=0, atol=1e-4)}\n"
                f"Parameters (no optimization): {np.allclose(u1_vanilla, u1_expected, rtol=0, atol=1e-4)}\n"
                f"Objective function (Armijo): {np.allclose(m1_armijo, m1_expected, rtol=0, atol=1e-4)}\n"
                f"Parameters (Armijo): {np.allclose(u1_armijo, u1_expected, rtol=0, atol=1e-4)}\n"
                "-- Model 4a: --\n"
